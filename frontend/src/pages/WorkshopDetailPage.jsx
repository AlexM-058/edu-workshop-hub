          ...
          <>
            <section className="mb-xl grid grid-cols-1 items-center gap-xl lg:grid-cols-12">
              <div className="lg:col-span-7">
                ...
                <div className="flex flex-wrap gap-4">
                  {workshop.is_open ? (
                    <button
                      className="rounded-xl bg-primary px-10 py-4 font-label-md text-on-primary shadow-lg shadow-primary/10 transition-all hover:opacity-95"
                      type="button"
                    ...
                    <span className="rounded-xl border border-outline-variant px-8 py-4 font-label-md text-on-surface-variant">
                      {locale === 'de' ? 'Keine freien Plätze' : 'Locuri epuizate'}
                    </span>
                  )}
                </div>
              </div>
                ...
                {workshop.referent && (
                  <div className="absolute -bottom-6 -left-6 rounded-xl border border-secondary/20 bg-secondary-container p-6 text-on-secondary-container shadow-xl">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-label-md">{workshop.referent.name}</p>
                        <p className="text-xs opacity-80">
                          {locale === 'de' ? 'Leitende Person' : 'Referent'}
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <div className="mb-xl grid grid-cols-1 gap-gutter md:grid-cols-3">
              ...
              <article className="flex flex-col gap-4 rounded-xl border border-outline-variant bg-surface-container-low p-8">
                <Stat
                  icon="event"
                  label={locale === 'de' ? 'Datum' : 'Data'}
                  value={dateStr}
                />
                <Stat
                  icon="location_on"
                  label={locale === 'de' ? 'Ort' : 'Locație'}
                  value={workshop.location}
                />
                <Stat
                  icon="group"
                  label={locale === 'de' ? 'Plätze' : 'Locuri'}
                  value={`${workshop.available_slots} ${locale === 'de' ? 'frei' : 'libere'} / ${workshop.max_slots}`}
                />
                <Stat
                  icon="circle"
                  label="Status"
